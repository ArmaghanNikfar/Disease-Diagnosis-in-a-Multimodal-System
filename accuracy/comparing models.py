import matplotlib.pyplot as plt


epochs = list(range(1, 26))
improved_LSTM_accuracy =  [ 0.4018,0.5049,0.6713,0.7053,0.7698,0.8154,0.8659,0.8031,0.8744,0.8936,0.9312,0.8728,0.9031,0.9073,0.8489,0.9333,0.9168,0.9731,
            0.9018, 0.9458,0.9921,0.9649,0.9581, 0.9290,0.9667 ]

simple_accuracy =[0.3070,0.4492,0.2664,0.3814,0.5418,0.6859 ,0.7139,0.6618,0.5574,0.6755,0.7462,0.7510 ,0.6820, 0.7632,00.8521,0.7432,0.7430,0.7820,
            0.8197,0.7983,0.8244,0.8853, 0.8395 , 0.7807, 0.9333]
Rnn_accuracy = [ 0.2587, 0.3293 , 0.3479, 0.4002, 0.3880, 0.4009,0.3495, 0.4378 , 0.4984, 0.4193,
            0.5500, 0.5938,  0.5721, 0.5668, 0.5692,0.6140,  0.6036, 0.5773, 0.6312, 0.6315,0.5964,0.5992, 0.6943,0.6783,0.8333]

mlp_accuracy = [0.2630,0.5189, 0.5548,0.5357, 0.6583,0.7438 ,0.8230 ,0.8743 , 0.8666,0.8675,0.9121 ,0.8558 ,0.8971, 0.8145, 0.9215 ,0.9381,
                0.9200,0.9368 ,0.9386,0.9846 ,0.9419 , 0.9459 ,0.9900 ,0.9484,0.9000]

plt.figure(figsize=(12,6))
plt.plot(epochs, improved_LSTM_accuracy , label="iLSTM", marker="o", linestyle="-", color="r")
plt.plot(epochs, simple_accuracy , label='sLSTM', marker='o', linestyle='-', color='b')
plt.plot(epochs, Rnn_accuracy , label='RNN', marker='o', linestyle='-', color='g')
plt.plot(epochs, mlp_accuracy , label='MLP', marker='o', linestyle='-', color='m')
plt.xlabel("Epochs")
plt.ylabel("Models Accuracy ")
plt.legend()
plt.grid(True)
plt.show()